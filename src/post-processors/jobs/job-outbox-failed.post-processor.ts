...
@Injectable()
export class JobOutboxFailedPostProcessor extends BatchPostProcessor<EventEventMessagingType> {
  constructor(
    redisClient: Redis,
  ...
  protected async processEvents(events: BatchEventItem<EventEventMessagingType>[]): Promise<void> {
    await Promise.all(
      events.map(async ({ event, messageId }) => {
        ...
        this.logger.info('Processing WS_JOBS_OUTBOX_FAILURE', {
          messageId,
          eventId: event.id,
          type: event.type,
        });
        ...
        if (originalPayload?.userId) {
          const wsPayload = { isEmitter: true, status: 'FAILED', ...originalPayload };
          ...
          this.logger.debug('No userId found in originalPayload for WS_JOBS_OUTBOX_FAILURE', {
            messageId,
            eventId: event.id,
          });
        }
